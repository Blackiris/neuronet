...
std::unique_ptr<NeuronsNetwork> NeuronsNetworkFactory::create_network(const unsigned int &input_size, const unsigned int &hidden_size, const unsigned int &output_size, const unsigned int &nb_layers) {
...
std::unique_ptr<NeuronsNetwork> NeuronsNetworkFactory::create_conv_network(const unsigned int &input_x, const unsigned int &input_y,
    ...
    neurons_network->m_layers.push_back(std::make_unique<OneToManyLayer>(std::move(conv_sub_layers)));
    neurons_network->m_layers.push_back(std::make_unique<MaxpoolLayer>(input_x-2, input_y-2, nb_features_map, 2));

    const std::vector<std::vector<unsigned>> links_tables = {{0,1,2}, {1,2,3}, {2,3,4},
                                                         {3,4,5}, {0,4,5}, {0,1,5},
                                                         {0,1,2,3}, {1,2,3,4}, {2,3,4,5},
                                                         {0,3,4,5}, {0,1,4,5}, {0,1,2,5},
                                                         {0,1,3,4}, {1,2,4,5}, {0,2,3,5},
                                                         {0,1,2,3,4,5}};
    std::vector<std::unique_ptr<INeuronsLayer>> conv2_sub_layers;
    unsigned sub_conv2_output_size = 0;
    for (unsigned int i=0; i<nb_features_map2; i++) {
        ...
        conv2_sub_layers.push_back(std::unique_ptr<ConvolutionLayer>(conv2_sub_layer));
        sub_conv2_output_size = conv2_sub_layer->get_output_size();
    }

    neurons_network->m_layers.push_back(std::make_unique<OneToManyLayer>(std::move(conv2_sub_layers)));
